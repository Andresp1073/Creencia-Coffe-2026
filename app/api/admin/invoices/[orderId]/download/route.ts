 ...
 */
function safeDownloadName(fileName: string, fallback: string, kind: InvoiceFileKind): string {
...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    if (kind !== "pdf" && kind !== "xml") {
      return NextResponse.json({ error: "kind debe ser 'pdf' o 'xml'" }, { status: 400 });
    }
    ...
    const fileName = safeDownloadName(file.file_name, `${invoice.number}.${kind}`, kind);
