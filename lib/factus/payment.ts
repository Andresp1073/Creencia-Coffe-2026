...
export function resolvePaymentData(raw: unknown): ResolvedPayment {
  ...
  const rawDueDate =
    typeof body.payment_due_date === "string" ? body.payment_due_date.trim() : "";
  ...
  if (form.code === "2") {
    if (!rawDueDate) {
      throw new ValidationError("payment_due_date es requerido para crédito (payment_form=2)");
    }
    if (!PAYMENT_DUE_DATE_RE.test(rawDueDate) || !isValidCalendarDate(rawDueDate)) {
      throw new ValidationError("payment_due_date debe tener formato YYYY-MM-DD y ser una fecha válida");
    }
    paymentDueDate = rawDueDate;
  }
